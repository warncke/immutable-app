...
internals.push(new RegExp(/bluebird\/js/))
internals.push(new RegExp(/node_modules\/express/))
...
function initAppDir (config, module, appDir, appSubDir) {
    ...
    _.each(files, file => {
        ...
        // models must be named model.js
            ...
            // check if model has already been defined

            // if model is not instantiated then it should be spec that can be
            route.model = model.ImmutableCoreModel ? model : new ImmutableCoreModel(model)
    ...
    // get access control provider for presenter
