...
function initAppDir (config, module, appDir, appSubDir) {
    ...
    // get mapping of templates path: name
    route.templatesInverse = _.invert(route.templates)
    ...
    // iterate over all route handlers
    _.each(controller.paths, path => {
        _.each(path, spec => {
            // get template for route - this can be either the name or the path
            ...
            if (route.templates[template]) {
                ...
                delete bareTemplates[template]
            }
            // template path exists locally
            if (route.templatesInverse[template]) {
                // template has a controller so remove from list
                delete bareTemplates[route.templatesInverse[template]]
            }
        })
    })
    // create paths for any templates without controllers
    _.each(bareTemplates, (templatePath, templateName) => {
        // create path
        ...
        // merge template to path
        _.merge(controller.paths, spec)
    })
    // create routes for controller paths
    _.each(controller.paths, (methods, path) => {
