...
function initAppDir (config, module, appDir, appSubDir) {
    ...
    _.each(fs.readdirSync(appSubDir), file => {
        ...
        // if this is plain file then see if it needs to be loaded
        if (stat.isFile()) {
            files.push(absFile)
        }
        // if this is directory then recurse
    ...
    // immutable controller module - there can be multiple controller files
    // but they must all export the same immutable-core-module
    route.controller
...
function initModels (config) {
    ...
    _.each(config.models, (model, modelName) => {
        ...
        if (!model.databaseObj) {
            ...
            var database = config.database[modelName] || config.database.default
            // require database
            assert.ok(database, 'database not found for model '+modelName)
