...
function initAppDir (config, module, appDir, appSubDir) {
    ...
    var templateRegEx = new RegExp('\\'+config.handlebars.extname+'$')
...
function initHandlebars (config) {
    ...
    var handlebarsConfig = {
        ...
        layoutsDir: _.reverse(config.dir.layouts)[0],
        partialsDir: _.reverse(config.dir.partials),
    }
    // debug handlebars config
    debug('init handlebars:', handlebarsConfig)
    // create new express handlebars instance
    var expressHandlebars = ExpressHandlebars.create(handlebarsConfig)
    // configure express
    config.express.engine(config.handlebars.extname, expressHandlebars.engine)
    config.express.set('view engine', config.handlebars.extname)
    ...
    config.express.set('views', _.reverse(viewsDir.concat(appDir))[0])
}
