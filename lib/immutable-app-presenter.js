...
function normalizeInput (data, field) {
    ...
    // clear nested array entries with no values set
...
function presenter (args, req, res, next) {
    ...
    var spec = _.find(args.spec, controller => {
        ...
        // otherwise if client has role then use controller
        else if (roles[controller.role]) {
            return true
        }
    })
